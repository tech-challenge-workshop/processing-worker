...
**Not yet in this branch's decision log.** `AD-009` (TypeORM), `AD-010` (only the outbox publishes) and `AD-011` (shared broker topology is a broker policy) were recorded on the `feat/durable-persistence` branch and are absent from `STATE.md` here, because this branch was cut from `main` before that merge. Two of them constrain this design and are honoured by content rather than by citation:
...
```mermaid
graph TD
    VQ[video.validation.requested] --> VC[ValidationConsumer]
    ...
    PQ[processing.queued] --> PC[ProcessingConsumer]
...
| `InMemoryDuplicateChecker` | `src/validation/in-memory-duplicate-checker.ts` | Unchanged. Event-level deduplication stays where it is; RM-13 adds object-level idempotency, which is a different question |
...
| Archive already at the key | `head` hit; key returned without extracting | Redelivery is invisible; no second object |
