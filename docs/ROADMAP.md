...
4. **Storage and events**: use deterministic S3 keys, store the ZIP, publish completed/failed outcomes with publisher confirms, and acknowledge only after the durable effect.
5. **Operations**: add container image tooling, queue-depth autoscaling support, observability, and integration tests with RabbitMQ/S3-compatible services.
